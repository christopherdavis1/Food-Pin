...
//  FoodPin
//
//  Created by Christopher Davis on 3/17/18.
...
class RestaurantTableViewController: UITableViewController {
    ...
    // Store Checked-In restaurants in an array, so we won't show the checkmark on restaurants that we haven't visited.
...
//    UNCOMMENT TO HIDE THE NAV ON SCROLL
    ...
    // MARK: - Pass the data in the view from the array
    ...
    // MARK: - Define data to hand via segue
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showRestaurantDetail" {
            if let indexPath = tableView.indexPathForSelectedRow {
                let desitinationController = segue.destination as! RestaurantDetailViewController
                desitinationController.restaurant = restaurants[indexPath.row]
            }
        }
    }

    
    // MARK: - Left-Swipe actions
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        
        // Create a delete action
        // .destructive gives it the default red color
        let deleteAction = UIContextualAction(style: .destructive, title: "Delete") { (action, sourceView, completionHandler) in
            // Tell it what to delete
            self.restaurants.remove(at: indexPath.row)
            
            // And how to delete it
            self.tableView.deleteRows(at: [indexPath], with: .fade)
            
            // This line checks to see if the action has been completed, and hides the action buttons.
            completionHandler(true)
        }
        
        // Create a Share action that opens the share sheet
        let shareAction = UIContextualAction(style: .normal, title: "Share") {(action, sourceView, completionHandler) in
            // If this item is shared via email or text, this will be the default text used.
            let defaultText = "Just checking in at " + self.restaurants[indexPath.row].name
            
            let activityController: UIActivityViewController
            
            // Define whether to share the image and text, or just text
            if let imageToShare = UIImage(named: self.restaurants[indexPath.row].image) {
                // If there's an image and text, share both.
                activityController = UIActivityViewController(activityItems: [imageToShare, defaultText], applicationActivities: nil)
            } else {
                // Otherwise, just share the text.
                activityController = UIActivityViewController(activityItems: [defaultText], applicationActivities: nil)
            }
            
            // On iPad, assign the share popover to the cell that the user taps on.
        ...
        // Set custom background colors and icons for the actions
        ...
        // Initialize both actions in the view
